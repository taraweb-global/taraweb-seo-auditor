"""
TaraWeb SEO Auditor — a local-first, concurrent technical SEO crawler.
...
def _env_int(name, default, minimum, maximum):
    """Read a bounded integer environment option without startup crashes."""
    try:
...
app.config['MAX_CONTENT_LENGTH'] = _env_int(
    'TARAWEB_MAX_REQUEST_BYTES', 16 * 1024 * 1024, 64 * 1024, 64 * 1024 * 1024
)
logging.basicConfig(level=logging.INFO, format='%(asctime)s [%(name)s] %(levelname)s: %(message)s')

APP_HOST = os.environ.get('TARAWEB_HOST', '127.0.0.1').strip() or '127.0.0.1'
APP_PORT = _env_int('TARAWEB_PORT', 5002, 1, 65535)
MAX_FETCH_BYTES = _env_int('TARAWEB_MAX_FETCH_BYTES', 10 * 1024 * 1024, 64 * 1024, 50 * 1024 * 1024)
ALLOW_PRIVATE_TARGETS = os.environ.get('TARAWEB_ALLOW_PRIVATE_TARGETS', '').lower() in ('1', 'true', 'yes')
ALLOW_INSECURE_TLS = os.environ.get('TARAWEB_ALLOW_INSECURE_TLS', '').lower() in ('1', 'true', 'yes')
# Browser-triggered source updates and process restarts are intentionally not
# supported in the public edition. Updates are an explicit terminal operation.
ENABLE_SELF_UPDATE = False
CSRF_TOKEN = os.environ.get('TARAWEB_CSRF_TOKEN') or secrets.token_urlsafe(32)
PRODUCT_NAME = 'TaraWeb SEO Auditor'
PRODUCT_DESCRIPTION = 'Local-first, open-source technical SEO crawling and website audit software by TaraWeb.'
PRODUCT_WEBSITE = 'https://taraweb.tech/'
PRODUCT_REPOSITORY = 'https://github.com/taraweb-global/taraweb-seo-auditor'
UPSTREAM_REPOSITORY = 'https://github.com/puneetindersingh/open-seo-crawler'


def _target_url_error(value):
    """Return a safe validation error for a user-supplied crawl target.

    Private, loopback, link-local and reserved addresses are blocked by
    default to prevent a browser user from turning a shared crawler into an
    SSRF proxy. Local/staging audits remain available through the explicit
    TARAWEB_ALLOW_PRIVATE_TARGETS=1 opt-in.
    """
    try:
        ...
        if host == 'localhost' or host.endswith('.localhost'):
            return 'Private and local network targets are blocked by default.'
        addresses = {info[4][0] for info in socket.getaddrinfo(host, parsed.port or 443, type=socket.SOCK_STREAM)}
        for address in addresses:
            ip = ipaddress.ip_address(address)
            if not ip.is_global:
                return 'Private, loopback, link-local and reserved targets are blocked by default.'
    except (OSError, ValueError):
        return 'The target hostname could not be safely resolved.'
    return None


class UnsafeTargetError(requests.exceptions.RequestException):
    """Raised before a request or redirect can reach a blocked address."""


class ResponseTooLargeError(requests.exceptions.RequestException):
    """Raised when a target exceeds the configured response-body limit."""


class SafeSession(requests.Session):
    """Requests session that validates the initial URL and every redirect hop.

    ``requests`` normally follows redirects internally, which means validating
    only the user-entered URL does not prevent a public host from redirecting
    the crawler to loopback or a private-network service. This session follows
    redirects manually and applies the same address policy before each request.
    """
    ...
    def __init__(self):
        super().__init__()
        # Do not implicitly consume proxy credentials or .netrc entries from
        # the host environment when crawling arbitrary third-party URLs.
        self.trust_env = False

    @staticmethod
    def _read_bounded(response, method):
        if method == 'HEAD':
            return response
        announced = response.headers.get('Content-Length', '')
        try:
            if announced and int(announced) > MAX_FETCH_BYTES:
                response.close()
                raise ResponseTooLargeError(
                    f'Response exceeds the {MAX_FETCH_BYTES}-byte download limit.'
                )
        except ValueError:
            pass
        chunks = []
    ...
    def request(self, method, url, **kwargs):
        ...
        for _ in range(self.max_redirects + 1):
            if error := _target_url_error(current_url):
                raise UnsafeTargetError(error)
            ...
            if not follow or response.status_code not in self._REDIRECT_CODES or not location:
                return self._read_bounded(response, current_method)

            next_url = urljoin(response.url, location)
            if error := _target_url_error(next_url):
                response.close()
                raise UnsafeTargetError(f'Blocked redirect target: {error}')
        ...
        raise requests.exceptions.TooManyRedirects(
            f'Exceeded the {self.max_redirects}-redirect limit.'
        )
...
def _protect_state_changes():
    if request.method in ('POST', 'PUT', 'PATCH', 'DELETE'):
        supplied = request.headers.get('X-TaraWeb-CSRF', '')
...
def _http_get(url, **kwargs):
    """Policy-enforced GET with an explicit opt-in for broken TLS sites."""
    try:
        r = SafeSession().get(url, **kwargs)
...
def _http_head(url, **kwargs):
    """Policy-enforced HEAD with the same explicit TLS opt-in."""
    try:
        r = SafeSession().head(url, **kwargs)
        r.ssl_bypassed = False
        return r
    except requests.exceptions.SSLError:
        if not ALLOW_INSECURE_TLS:
...
def _local_commit_sha():
    """Return a non-executable build label for the public edition."""
    return 'dev'
...
def _cb_fetch_sitemap_urls(base_url, ua):
    ...
    def parse(sm, depth=0):
        ...
        seen.add(sm)
        try:
            r = _http_get(sm, headers=ua, timeout=15)
    ...
    robots_sitemaps = []
    try:
        r = _http_get(hp + '/robots.txt', headers=ua, timeout=10)
        if r.status_code == 200:
...
def _analyze_crawl_budget(start_url, max_pages=14, timeout=10):
    ...
    def grab(url):
        try:
            r = _http_get(url, headers=ua, timeout=timeout)
...
def crawl_budget_analyze():
    ...
    if not url.startswith(('http://', 'https://')):
        url = 'https://' + url
    if error := _target_url_error(url):
...
def version():
    ...
    return jsonify({
        ...
        'repo': 'taraweb-global/taraweb-seo-auditor',
    })


@app.route('/update', methods=['POST'])
def update_self():
    """Refuse browser-triggered source mutation in the public edition."""
    return jsonify({
        'ok': False,
        'error': 'Browser-based updates are unavailable. Review and apply updates manually with Git.',
    }), 410


@app.route('/restart', methods=['POST'])
def restart_self():
    """Refuse browser-triggered process control in the public edition."""
    return jsonify({
        'ok': False,
        'error': 'Browser-based restart is unavailable. Restart the local process explicitly.',
    }), 410
...
def index():
    return render_template(
        ...
        self_update_enabled=ENABLE_SELF_UPDATE,
...
def _crawl_page(url, session, domain, pw_page=None, ignore_noindex=False, capture_no_js=False, challenge_browser=None):
    ...
    try:
        ...
        for attempt in range(3):  # 1 primary + 2 retries
            try:
                ...
                if not ALLOW_INSECURE_TLS:
                    resp = None
                    break
                # Explicit operator opt-in for auditing a known site with a
                # broken/self-signed chain. This is never enabled by default.
                try:
                    import urllib3
                    urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
                except Exception:
        ...
        result['body_hash'] = _hashlib.sha256(_norm.encode('utf-8', errors='ignore')).hexdigest() if _norm else ''
...
def fetch_robots_txt():
    ...
    if error := _target_url_error(target):
        return jsonify({'error': error}), 400
    try:
        ...
        resp = _http_get(robots_url, timeout=10,
                            headers={'User-Agent': 'Mozilla/5.0 (compatible; TaraWebSEOAuditor-RobotsPreview)'},
...
def sitemap_analyse():
    ...
    if error := _target_url_error(domain):
        return jsonify({'error': error}), 400

    if manual_sm:
        if not manual_sm.startswith('http'):
            manual_sm = 'https://' + manual_sm.lstrip('/')
        if error := _target_url_error(manual_sm):
            return jsonify({'error': error}), 400
...
def near_dup_content():
    ...
    if not isinstance(pages, list) or len(pages) > 10000 or any(not isinstance(page, dict) for page in pages):
        return jsonify({'error': 'Pages must contain at most 10,000 page objects.'}), 400
    try:
...
def crawl_site():
    ...
    if resumed_state:
        ...
        try:
            max_pages = _bounded_int(data.get('max_pages') or cfg.get('max_pages'), 500, 1, 999999, 'Max pages')
            max_depth = _bounded_int(cfg.get('max_depth'), 10, 1, 20, 'Max depth')
            crawl_delay = _bounded_float(cfg.get('crawl_delay'), 0.4, 0.0, 3.0, 'Per-host delay')
        ...
        try:
            max_pages = _bounded_int(data.get('max_pages'), 500, 1, 5000, 'Max pages')
            max_depth = _bounded_int(data.get('max_depth'), 10, 1, 20, 'Max depth')
            crawl_delay = _bounded_float(data.get('crawl_delay'), 0.4, 0.0, 3.0, 'Per-host delay')
        except ValueError as exc:
            return jsonify({'error': str(exc)}), 400
        if max_pages >= 5000:
    ...
    if error := _target_url_error(seed_url):
        return jsonify({'error': error}), 400
    ...
    try:
        max_workers = _bounded_int(data.get('max_workers'), 5, 1, 20, 'Workers')
    except ValueError as exc:
        return jsonify({'error': str(exc)}), 400
    if render_js:
        max_workers = 1
    ...
    def generate():
        ...
        if render_js:
            try:
                ...
                pw_browser = pw_ctx.chromium.launch(headless=True, args=['--disable-dev-shm-usage'])
                pw_page = pw_browser.new_page(
                    viewport={'width': 1280, 'height': 900},
                    user_agent=crawl_ua,
                )
                def _guard_browser_request(route):
                    if _target_url_error(route.request.url):
                        route.abort('blockedbyclient')
                    else:
                        route.continue_()
                pw_page.route('**/*', _guard_browser_request)
...
# union.
_CRAWL_FOLDERS_RO = [os.path.realpath(os.path.expanduser(p)) for p in
...
def crawl_save():
    ...
    if len(results) > 50000 or any(not isinstance(row, dict) for row in results):
        return jsonify({'error': 'Crawl results must contain at most 50,000 page objects.'}), 400
    if not isinstance(inlinks, dict) or not isinstance(reports, dict):
        return jsonify({'error': 'Invalid crawl metadata.'}), 400
    os.makedirs(_CRAWL_FOLDER, exist_ok=True)
...
_CRAWL_FILENAME_RE = _re.compile(r'^[A-Za-z0-9][A-Za-z0-9._-]{0,199}\.json$')


def _safe_crawl_file(folder, filename, must_exist=True):
    """Resolve a regular JSON crawl file without traversal or symlink escape."""
    if not isinstance(filename, str) or not _CRAWL_FILENAME_RE.fullmatch(filename):
        return None
    ...
    try:
        ...
        return None
    if must_exist and (not os.path.isfile(candidate) or os.path.islink(os.path.join(base, filename))):
        return None
    return candidate
...
def crawl_load():
    ...
    if not _CRAWL_FILENAME_RE.fullmatch(fn):
...
def crawl_delete():
    ...
    if not isinstance(fn, str) or not _CRAWL_FILENAME_RE.fullmatch(fn):
        return jsonify({'error': 'Invalid file'}), 400
    # Extra crawl folders are documented as read-only and must never be
    # mutated through the application.
    path = _safe_crawl_file(_CRAWL_FOLDER, fn)
    if not path:
        return jsonify({'error': 'Not found'}), 404
    try:
        os.remove(path)
...
if __name__ == "__main__":
    if APP_HOST not in ('127.0.0.1', '::1', 'localhost'):
        app.logger.warning('Listening beyond localhost. Use a firewall and trusted network only.')
